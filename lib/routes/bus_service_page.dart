...
//  ScrollController controller;
  ...
  Future<void> initService() async {
    ...
    if (widget.focusedBusStop != null) {
      final BusServiceRoute focusedRoute = service.routes[0].busStops.contains(widget.focusedBusStop) ? service.routes[0] : service.routes[1];
...
//      controller = ScrollController(initialScrollOffset: 56 * index);
  ...
  Widget build(BuildContext context) {
    ...
    final Widget bottomSheetContainer = bottomSheet(child: _buildBody(tabController));
          ...
          SliverOverlapAbsorber(
            ...
            sliver: SliverAppBar(
              ...
              flexibleSpace: FlexibleSpaceBar(
                titlePadding: EdgeInsets.only(left: 72, bottom: (service?.directionCount ?? 0) > 1 ? kTextTabBarHeight : 16.0),
            ...
            slivers: <Widget>[
              ...
              SliverList(
                delegate: SliverChildBuilderDelegate(
                    ...
                    final String previousRoad = position > 0 ? route.busStops[position - 1].road : '';
                      ...
                      children: <Widget>[
                        Positioned.fill(
                          child: Row(
                            children: <Widget>[
                              Container(
                                margin: position == 0 ? const EdgeInsets.only(left: 36.0, top: 72.0) : position < route.busStops.length - 1 ? const EdgeInsets.only(left: 36.0) : const EdgeInsets.only(left: 36.0, bottom: 8.0),
                                child: Container(
                        ...
                        Column(
                          ...
                          children: <Widget>[
                            if (newRoad)
                              Padding(
                                padding: const EdgeInsets.only(left: 88.0, top: 24.0, bottom: 8.0),
                                child: Text(busStop.road, style: Theme.of(context).textTheme.headline4),
                              ),
                            Material(
                              color: busStop == widget.focusedBusStop ? focusedColor : Colors.transparent,
                              child: InkWell(
                                onTap: () => showBusDetailSheet(busStop, UserRoute.home),
                                child: ListTile(
                                  contentPadding: const EdgeInsets.symmetric(horizontal: 8.0),
                                  title: Text('${busStop.defaultName}', style: Theme.of(context).textTheme.headline6),
                                  subtitle: Text('${busStop.code}', style: Theme.of(context).textTheme.subtitle2.copyWith(color: Theme.of(context).hintColor)),
                                  leading: Ink(
                                    color: Color.alphaBlend(busStop == widget.focusedBusStop ? focusedColor : Colors.transparent, Theme.of(context).scaffoldBackgroundColor),
                                    width: 64.0,
                                    height: 72.0,
                                    child: Column(
